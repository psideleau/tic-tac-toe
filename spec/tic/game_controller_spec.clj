...
(describe "user playing game"
    ...
    (it "should allow user to take free square and have computer go next"
        ...
        (should= :O (board/get-board-square (.board game-state) 1))
        (should= 1 (count (board/taken-squares (.board game-state) :X)))
        (should= :O (:current-turn (.game game-state)))
        (should (:update-board-called @game-spy-data))
        (should-not (.tie? game-state))
        (should-not (.winner game-state))
        (should-not (.loser game-state))
    ...
    (it "should throw an exception if user attempts to take a square already taken"
        (game-controller/take-square! game-state 1 spy-game-listener)
        (should-throw IllegalStateException (game-controller/take-square! game-state 1)))
    ...
    (it "a computer should beat an inexperienced user"
        ...
        (should (board/winner? (.board game-state) (:computer (.game game-state))))
        (should= :X (board/winner (.board game-state)))
        (should= :X (.winner game-state))
        (should= :O (.loser game-state))
        (should-not (.tie? game-state))
